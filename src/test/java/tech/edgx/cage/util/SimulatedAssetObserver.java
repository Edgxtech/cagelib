...
public class SimulatedAssetObserver extends TimerTask {
    ...
    public void run() {
        ...
        cageProcessManager.getGeoMission().getTarget().setTrue_current_loc(new Double[]{true_lat,true_lon});
...
//            double a_x = utm_coords[1];
    ...
    //                    }
                        ...
                        cageProcessManager.addObservation(obs);
                    }
                    ...
                    if (asset.getProvide_tdoa() != null && asset.getProvide_tdoa() && asset.getTdoa_asset_ids() != null && !asset.getTdoa_asset_ids().isEmpty()) {
                        ...
                        for (String secondary_asset_id : asset.getTdoa_asset_ids()) {
                            ...
                            *  for sim use the upper limit assuming full measurement error */
                            ...
                            cageProcessManager.addObservation(obs_c);
                        }
                    }
                    ...
                    if (asset.getProvide_aoa() != null && asset.getProvide_aoa()) {
                        ...
                        cageProcessManager.addObservation(obs_d);
                    }
                } catch (Exception e) {
    ...
    public CageProcessManager getEfusionProcessManager() {
        return cageProcessManager;
    }

    public void setEfusionProcessManager(CageProcessManager cageProcessManager) {
